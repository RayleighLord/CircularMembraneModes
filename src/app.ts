...
import { modeShapeTex, renderMath } from "./ui/math";
...
export function startApp(): void {
  ...
  const selectors = new ModeSelectors(getElement<HTMLElement>("mode-selectors"), {
    ...
    onCommit: ({ values }) => {
      announce(
        interactionStatus,
        `Selected azimuthal mode m ${values.m}, radial mode n ${values.n}. ` + nodalSummary(values)
      );
    }
  });
  ...
  function dispatch(action: Parameters<typeof reduceControllerState>[1]): void {
    ...
    const modeChanged =
      previous.mode.m !== next.mode.m || previous.mode.n !== next.mode.n;
    if (modeChanged) {
  ...
  function renderMode(mode: ModeSelection): void {
    ...
    description.textContent =
      `Animated circular-membrane mode m ${mode.m}, n ${mode.n}. ` +
      "Height and the Berlin blue-to-coral color scale show instantaneous signed displacement. " +
      "A polar grid follows the deforming surface. " +
      `Its spatial pattern has ${pattern.circleCount} ${plural(pattern.circleCount, "interior nodal circle")} ` +
      `and ${pattern.diameterCount} ${plural(pattern.diameterCount, "nodal diameter")}. ` +
      "The pale circular perimeter is fixed at zero displacement. " +
      (mode.m > 0
        ? "A sine partner gives the same pattern rotated around the center."
        : "This axisymmetric mode has no distinct sine partner.");
  }
...
function nodalSummary(mode: ModeSelection): string {
  const pattern = nodalPattern(mode);
  return (
    `${pattern.circleCount} ${plural(pattern.circleCount, "interior nodal circle")} and ` +
    `${pattern.diameterCount} ${plural(pattern.diameterCount, "nodal diameter")}`
  );
}
