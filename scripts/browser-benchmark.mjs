...
try {
  ...
  const resourcesBefore = await readResources(page);
  const modeLatencies = [];
  for (let repeat = 0; repeat < 3; repeat += 1) {
    for (const [m, n] of [[0, 1], [8, 8], [2, 7], [7, 2], [4, 5], [2, 3]]) {
      const start = performance.now();
      await page.locator("#m-slider").fill(`${m}`);
      await page.locator("#n-slider").fill(`${n}`);
      await page.waitForFunction(
        ([azimuthal, radial]) => document.querySelector("#plate-stage")?.getAttribute("data-mode") === `${azimuthal},${radial}`,
        [m, n]
      );
  ...
  assert.equal(animation.geometries, 2, "Renderer should retain only the disk and perimeter geometries");
  assert.ok(animation.textures >= 1 && animation.textures <= 4, "Texture count grew unexpectedly");
  assert.deepEqual(
    { geometries: animation.geometries, textures: animation.textures, programs: animation.programs },
    resourcesBefore,
    "Mode changes must not allocate additional retained GPU resources"
  );
...
async function readResources(page) {
  return page.locator("#plate-stage").evaluate((stage) => ({
    geometries: Number(stage.getAttribute("data-geometry-count")),
    textures: Number(stage.getAttribute("data-texture-count")),
    programs: Number(stage.getAttribute("data-program-count"))
  }));
