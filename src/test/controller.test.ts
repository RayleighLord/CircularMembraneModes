...
describe("immutable explorer controller", () => {
  ...
  it("updates one mode index without mutating the previous state", () => {
    ...
    expect(updated.mode).toEqual({ m: 7, n: 3 });
    expect(initial.mode).toEqual({ m: 2, n: 3 });
    expect(Object.isFrozen(initial)).toBe(true);
    expect(Object.isFrozen(updated)).toBe(true);
    expect(Object.isFrozen(updated.mode)).toBe(true);
  });

  it("accepts the axisymmetric m=0 mode and updates the radial index independently", () => {
    const initial = createInitialControllerState();
    const axisymmetric = reduceControllerState(initial, {
      type: "set-mode-index",
      axis: "m",
      value: 0
    });
    const radial = reduceControllerState(axisymmetric, {
      type: "set-mode-index",
      axis: "n",
      value: 8
    });

    expect(axisymmetric.mode).toEqual({ m: 0, n: 3 });
    expect(radial.mode).toEqual({ m: 0, n: 8 });
  });
  ...
  it("resets mode, rate, visibility, and playback while preserving motion preference", () => {
    let state = createInitialControllerState({ prefersReducedMotion: true, mode: { m: 8, n: 7 } });
