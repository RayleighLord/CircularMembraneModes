...
const repositoryPath = "/CircularMembraneModes/";
...
try {
  ...
  console.log("Browser smoke checks passed for all 72 circular-membrane modes.");
} finally {
  await browser?.close();
...
async function assertInitialComposition(page) {
  assert.equal(await page.title(), "Circular Membrane Modes");
  assert.equal(await page.locator('[data-membrane-canvas="true"]').count(), 1);
  assert.equal(await page.locator("#m-slider").getAttribute("min"), "0");
  assert.equal(await page.locator("#m-slider").getAttribute("max"), "8");
  assert.equal(await page.locator("#m-slider").inputValue(), "2");
  assert.equal(await page.locator("#n-slider").getAttribute("min"), "1");
  assert.equal(await page.locator("#n-slider").getAttribute("max"), "8");
  assert.equal(await page.locator("#n-slider").inputValue(), "3");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-circle-count"), "2");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-diameter-count"), "2");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-overlay"), "false");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-radial-rings"), "128");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-angular-sectors"), "256");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-vertex-count"), "32769");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-triangle-count"), "65280");
  ...
  assert.ok(
    Math.abs(Number(await page.locator("#plate-stage").getAttribute("data-frequency-ratio")) -
      4.8318852629306) < 1e-12
  );
  assert.ok(
    Math.abs(Number(await page.locator("#plate-stage").getAttribute("data-cycle-seconds")) -
      2.0695855666769027) < 1e-12
  );
  const formulaTex = await page.locator("#shape-math annotation").textContent();
  assert.match(formulaTex, /\\phi_\{m,n\}\(r,\\theta\)=J_m/);
  assert.match(formulaTex, /j_\{m,n\}r\/R/);
  assert.match(formulaTex, /\\cos\(m\\theta\)/);
  assert.doesNotMatch(formulaTex, /widehat|A_\{m,n\}|\\lVert|\\omega|frequency/i);
  assert.equal(await page.locator("#frequency-math").count(), 0);
  ...
  const targets = await page.locator("#m-slider, #n-slider, #reset-camera, #ui-visibility-toggle, #animation-toggle").evaluateAll(
  ...
  assert.ok(layout.formulaLeft >= layout.headerRight, "Formula overlaps the title at desktop size");
  assert.ok(layout.formulaRight <= layout.viewControlsLeft, "Formula overlaps the view controls");
  await assertFormulaFits(page);
  ...
  await page.evaluate(() => {
    const input = document.querySelector("#m-slider");
    input.value = "1";
  ...
  if (process.env.UPDATE_README_SCREENSHOT === "1") {
    await page.screenshot({
      path: new URL("circular-membrane-modes-explorer.png", docsDir).pathname,
      fullPage: true
    });
  }
  await page.locator("#animation-toggle").click();
...
async function assertModeSelection(page) {
  const m = page.locator("#m-slider");
  const n = page.locator("#n-slider");

  await m.focus();
  await m.press("Home");
  await waitForMembrane(page, 0, 3);
  assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-diameter-count"), "0");
  assert.match(await page.locator("#plate-description").textContent(), /axisymmetric mode/i);
  ...
  assert.match(await page.locator("#shape-math annotation").textContent(), /\\phi_\{m,n\}/);
  assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-circle-count"), "4");
  assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-diameter-count"), "2");
  assert.match(await page.locator("#plate-description").textContent(), /4 interior nodal circles/);
  assert.match(await page.locator("#plate-description").textContent(), /sine partner/i);
...
async function assertAllModes(page) {
  ...
  await toggle.click();
  for (let m = 0; m <= 8; m += 1) {
    await page.locator("#m-slider").fill(`${m}`);
    for (let n = 1; n <= 8; n += 1) {
      await page.locator("#n-slider").fill(`${n}`);
      await waitForMembrane(page, m, n);
      assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-circle-count"), `${n - 1}`);
      assert.equal(await page.locator("#plate-stage").getAttribute("data-nodal-diameter-count"), `${m}`);
      const frequencyRatio = Number(
        await page.locator("#plate-stage").getAttribute("data-frequency-ratio")
      );
      const cycleSeconds = Number(
        await page.locator("#plate-stage").getAttribute("data-cycle-seconds")
      );
      assert.ok(Number.isFinite(frequencyRatio) && frequencyRatio >= 1);
      assert.ok(Number.isFinite(cycleSeconds) && cycleSeconds > 0);
      assert.ok(
        Math.abs(frequencyRatio * cycleSeconds - 10) < 1e-10,
        `Mode (${m}, ${n}) does not preserve the ten-second fundamental calibration`
      );
      if ((m === 0 && n === 1) || (m === 1 && n === 1) ||
          (m === 2 && n === 3) || (m === 8 && n === 8)) {
        await page.screenshot({
          path: new URL(`mode-${m}-${n}-maximum.png`, artifactDir).pathname
        });
      }
    }
  }
  await page.locator("#m-slider").fill("2");
  await page.locator("#n-slider").fill("3");
  await waitForMembrane(page, 2, 3);
  await toggle.click();
}
...
async function assertPersistedPageLifecycle(page) {
  ...
  await page.locator("#m-slider").fill("3");
...
async function waitForMembrane(page, m, n) {
...
async function readLayout(page) {
  return page.evaluate(() => {
    ...
    const formula = document.querySelector(".formula-card").getBoundingClientRect();
    const header = document.querySelector(".mode-controls__header").getBoundingClientRect();
    ...
    return {
      ...
      viewControlsLeft: viewControls.left,
      documentWidth: document.documentElement.scrollWidth,
...
async function assertFormulaFits(page) {
  const metrics = await page.locator("#formula-card").evaluate((element) => {
    const rect = element.getBoundingClientRect();
    return {
      left: rect.left,
      right: rect.right,
  ...
  assert.ok(metrics.left >= -1 && metrics.right <= metrics.viewportWidth + 1, "Formula card is clipped");
  assert.ok(metrics.scrollWidth <= metrics.clientWidth + 1, "Formula content overflows its card");
}
